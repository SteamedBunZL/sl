...
public class VCardComposer {
	...
	public String createVCard(ContactStruct struct, int vcardversion)
		...
		if (struct.photoBytes != null) {
			if(Utils.debug)Log.i(TAG, struct.photoBytes + "struct.photoBytes"
					+ struct.photoType);
		...
		if (struct.extensionMap!=null&&struct.extensionMap.size() > 0) {
//			  for (Entry<String, List<String>> extension : struct.extensionMap.entrySet())
//	            {
//	                if (extension.getValue() != null && extension.getValue().size() > 0)
//	                    mResult.append(extension.getKey().toUpperCase() + ":").append(
//	                            foldingString(extension.getValue().get(0), vcardversion)).append(mNewline);
//	            }
            for (Entry<String, List<String>> extension : struct.extensionMap.entrySet())
            {
                if (extension.getValue() != null && extension.getValue().size() > 0){                	
                	for(String str:extension.getValue()){
                		mResult.append( extension.getKey().toUpperCase() + ":");
                		mResult.append(
                                foldingString(str, vcardversion));
	...
	private String foldingString1(String str, int version) {
		if (str.endsWith("\r\n")) {
			str = str.substring(0, str.length() - 2);
		} else if (str.endsWith("\n")) {
			str = str.substring(0, str.length() - 1);
	...
	private void appendPhotoStr(byte[] bytes, String type, int version)
		...
		mResult.append(encodingStr).append(";TYPE=").append(type).append(":")
				.append(value).append(mNewline);
	...
	private void appendContactMethodStr(
		...
		for (ContactStruct.ContactMethod contactMethod : contactMList) {
			...
			case Contacts.KIND_POSTAL:
				if(Utils.debug)Log.i(TAG, "++++++++++++++++++++++++++++++++++++++++++++++++++++++"+Contacts.KIND_POSTAL);
				if (!isNull(contactMethod.data)) {					
					mResult.append("ADR;TYPE=POSTAL:")
					        .append(";;")
							.append(foldingString1(contactMethod.data, version)).
							append(";;;;").append(mNewline);
					if(Utils.debug)Log.i(TAG, foldingString1(contactMethod.data, version));
				}
				break;
			default:
				break;
