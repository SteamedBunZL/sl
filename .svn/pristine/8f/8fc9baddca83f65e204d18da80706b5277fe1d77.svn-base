...
public class ContactStruct {
    ...
    public static class PhoneData {
        ...
        public String toString() {
    		return String.format("@%s: %s: %s: %s", type,data,isPrimary,label);
    	}
    }
    ...
    public static class ContactMethod {
        ...
        public boolean isPrimary;
        public String toString() {
    		return String.format("@%s: %s: %s: %s: %s", type, kind,data,isPrimary,label);
    	}
    }
    ...
    public static class OrganizationData {
        ...
        public boolean isPrimary;
        public String toString() {
    		return String.format("@%s: %s: %s: %s", type, companyName,positionName,isPrimary);
    	}
    }
    ...
    public void addExtension(PropertyNode propertyNode) {
         ...
         list.add(propertyNode.propValue);
    }
    ...
    public static ContactStruct constructContactFromVNode(VNode node,
        ...
        for (PropertyNode propertyNode: node.propList) {
            ...
            if (name.equals("VERSION")) {
                ...
                //              ; Code, Country Name
                if(Utils.debug)
                android.util.Log.i(LOG_TAG, propertyNode.propValue);
                if(Utils.debug)
                for(String str:propertyNode.paramMap_TYPE)
                	android.util.Log.i(LOG_TAG, str);
                ContentValues cv=propertyNode.paramMap;
                if(Utils.debug)
                android.util.Log.i(LOG_TAG, cv.toString());
                String address;
                ...
                if(Utils.debug)
                android.util.Log.i(LOG_TAG,address);
            	...
            	if(Utils.debug)
            	android.util.Log.i(LOG_TAG, "photo");
                ...
                if (valueType != null && valueType.equals("URL")) {
                	...
                	if(Utils.debug)android.util.Log.i(LOG_TAG, "propValue+++++"+propertyNode.propValue);
                	
                	ContentValues cv=propertyNode.paramMap;
                	if(Utils.debug)android.util.Log.i(LOG_TAG, "paramMap+++++"+cv.toString());
                	
                	if(Utils.debug)android.util.Log.i(LOG_TAG, "propValue_bytes+++++"+propertyNode.propValue_bytes);
                	contact.photoBytes = propertyNode.propValue_bytes;
                    String type = propertyNode.paramMap.getAsString("TYPE");
                    if(Utils.debug)android.util.Log.i(LOG_TAG, "TYPE+++++"+type);
                    if (type != null) {
                        contact.photoType = type;
                    }else{
                    	for(String str:propertyNode.paramMap_TYPE){
                    		if(Utils.debug)android.util.Log.i(LOG_TAG, "paramMap_TYPE+++++"+str);
                    		type=str;
                    		if (type != null) {
                                contact.photoType = type;
                            }
