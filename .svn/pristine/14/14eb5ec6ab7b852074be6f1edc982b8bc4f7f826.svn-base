...
public class AlbumGallery extends Gallery {
	...
	public AlbumGallery(Context context, AttributeSet attrs) {
		...
		this.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				if (view instanceof AlbumImageView) {
					imageView = (AlbumImageView) view;
					...
					if (event.getAction() == MotionEvent.ACTION_MOVE) {
						...
						if (event.getPointerCount() == 2) {
							...
							if (baseValue == 0) {
								...
								// scale the image
								imageView.zoomTo(originalScale * scale, x + event.getX(1), y
										+ event.getY(1));
	...
	public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
		...
		if (view instanceof AlbumImageView) {
			...
			if ((int) width <= Utils.SreenW && (int) height <= Utils.SreenH)// 如果图片当前大小<屏幕大小，直接处理滑屏事件
			{
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (view instanceof AlbumImageView) {
				...
				if ((int) width <= Utils.SreenW && (int) height <= Utils.SreenH)// 如果图片当前大小<屏幕大小，判断边界
				{
					break;
				}
				float v[] = new float[9];
				...
				if (Utils.debug)Log.i(TAG,"lyc---bottom:" + bottom);
				if (bottom < Utils.SreenH) {
					imageView.postTranslateDur(Utils.SreenH - bottom, 200f);
				}
			}
			break;
		}
		...
		public boolean onDoubleTap(MotionEvent e) {
			...
			if (view instanceof AlbumImageView) {
				...
				if (imageView.getScale() > imageView.getScaleRate()) {
					imageView.zoomTo(imageView.getScaleRate(), Utils.SreenW / 2, Utils.SreenH / 2,
							200f);
					// imageView.layoutToCenter();
				} else {
					imageView.zoomTo(1.0f, Utils.SreenW / 2, Utils.SreenH / 2, 200f);
				}
			} else {
			}
