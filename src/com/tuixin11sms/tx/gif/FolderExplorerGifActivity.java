...
public class FolderExplorerGifActivity extends BaseActivity implements
		OnClickListener {

	private static final String TAG = "FolderExplorerGifActivity";
	...
	public static final String GIF_PACKAGE_ID = "gif_package_id";
	...
	public static final int MAX_MAX_SELECTED_NUM = 1;// 可以同时勾选的文件上限个数
	...
	private GifTranscoldMgr tsm;

	public void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		getWindow().requestFeature(Window.FEATURE_NO_TITLE);
		setContentView(R.layout.activity_gif_sdcard);
		tsm = GifTranscoldMgr.getInstance();
		intent = getIntent();
		...
		mIsSendFiles = intent.getBooleanExtra(
				FolderExplorerGifActivity.IS_SEND_FILES, false);
		mTitleBar = (TextView) findViewById(R.id.tv_title_sdcard_name);
		mTitleBar.setText(mTitleText);
		// 要显示该目录下面指定类型的文件
		mFileMang = new FileManager(mSess.getContext(), fileType,
				folder_root_path);
		...
		btn_back_left.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				FolderExplorerGifActivity.this.finish();
			}
		});
		...
		lv_files_list
				.setOnItemClickListener(new AdapterView.OnItemClickListener() {
					...
					public void onItemClick(AdapterView<?> parent, View view,
						...
						if (resultCode == FolderExplorerGifActivity.ERROR_CANNOT_READ_DERECTORY) {
							// 无法打开此文件夹
							showToast("无法查看此文件夹，权限不足");
						} else if (resultCode == FolderExplorerGifActivity.ERROR_REACH_MAX_SELECTED_NUM) {
							// 选择文件数到达上限
							showToast("每次最多选择"
									+ FolderExplorerGifActivity.MAX_MAX_SELECTED_NUM
									+ "个文件");
						}
					}
				});
	 ...
	 * Main activity.
	 */
	String select_gif_package = null;
	...
	public void onClick(View v) {
		...
		case R.id.tv_send_selected_files:
			...
			if (mMultiSelectData != null && mMultiSelectData.size() > 0) {
				select_gif_package = mMultiSelectData.get(0).toString();
			} else {
				return;
			}
			// 判断选中的文件路径是否合法
			if (!mFileMang.isFileGifPackage(this,
					FileManager.getFileSuffix(select_gif_package))) {
				Toast.makeText(thisContext, "文件不可添加", Toast.LENGTH_SHORT)
						.show();
				return;
			}
			// 检测用户是否已经添加过这个包
			if (!IsGifPackageValid(select_gif_package)) {
				Toast.makeText(thisContext, "表情包已存在", Toast.LENGTH_SHORT)
						.show();
				return;
			}
			// 把文件拷到/shenliao/gif_pagkage TODO 以后改为线程池
			// 先把包名改了 包名为 包id_包md5_包名.eif

			new Thread() {
				public void run() {
					FileManager.copyFile(select_gif_package);
				};
			}.start();
			// 把缩略图解开，并保存在本地,并保存在SP中
			tsm.saveGifThumbnailToSDCard(select_gif_package,
					FileManager.getShenLiaoGifImgPath(),
					FileManager.getFileMD5(select_gif_package), -1, false);

			// 开启线程上传这个包

			Utils.uploadGifBag(mMultiSelectData.get(0),
					new GifDownUploadListner() {
						...
						public void onError(GifTaskInfo taskInfo, int iCode,
							...
							case GifTransfer.DUERR_INVALIDPARA:
								Log.e(TAG, "===任务无效");
								break;
							case GifTransfer.DUERR_SERVERREJECT:
								Log.e(TAG, "===服务器拒绝");
								break;
							default:
								Log.e(TAG, "===上传失败");
			...
			data.putStringArrayListExtra(BaseMsgRoom.MSG_FILE_LIST,
					mMultiSelectData);
			// data.putExtra(GIF_PACKAGE_ID, -1);
			setResult(Activity.RESULT_OK, data);
			// saveToSP(select_gif_package);
			finish();

			// if (mMultiSelectData != null && mMultiSelectData.size() != 0) {
			// if (!mFileMang.isFileGifPackage(
			// thisContext,
			// mMultiSelectData
			// .get(0)
			// .toString()
			// .substring(
			// mMultiSelectData.get(0).toString()
			// .lastIndexOf(".") + 1))) {
			// Toast.makeText(thisContext, "文件不可添加", Toast.LENGTH_SHORT)
			// .show();
			// return;
			...
			// }
			// // 检测用户是否已经添加过这个包
			// if (!IsGifPackageValid(mMultiSelectData.get(0).toString())) {
			// Toast.makeText(thisContext, "表情包已存在", Toast.LENGTH_SHORT)
			// .show();
			// return;
			// }
			// // 把文件拷到/shenliao/gif/下
			// new Thread() {
			// public void run() {
			// FileManager.copyFile(mMultiSelectData.get(0));
			// };
			// }.start();
			// // TODO 开启线程后台上传表情包？
			// new Thread() {
			...
			//
			// @Override
			// public void onError(GifTaskInfo taskInfo,
			...
			// }
			// }.start();
			// // tsm.saveGifList(mMultiSelectData.get(0));
			// Intent data = new Intent();
			// // 返回上一级页面一个路径
			// data.putStringArrayListExtra(BaseMsgRoom.MSG_FILE_LIST,
			// mMultiSelectData);
			// setResult(Activity.RESULT_OK, data);
			// saveToSP(mMultiSelectData.get(0));
			// finish();
			break;
		}
	}
	 ...
	 * 
	 * @param path
	 *            用户添加的地址 不是shenliao下的
	 */
	public void saveToSP(String path) {
		tsm.getGifMessage2(path);
	}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (mIsSendFiles) {
				if (mMultiSelectData != null
					...
					mViewHolder.mSelect.setImageResource(R.drawable.file_sel);
				else
					mViewHolder.mSelect.setImageResource(R.drawable.file_unsel);
			} else {
